...
suite("Place API tests", () => {
  ...
  setup(async () => {
    ...
    kilkennyPlaces = await placemarkService.createCategory(kilkenny);
  });
  ...
  test("denormalised category", async () => {
    ...
    const returnedCategory = await placemarkService.getCategory(kilkennyPlaces._id);
    assert.equal(returnedCategory.places.length, testPlaces.length);
    for (let i = 0; i < testPlaces.length; i += 1) {
      assertSubset(testPlaces[i], returnedCategory.places[i]);
    }
  });
});
